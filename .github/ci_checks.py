...
CAPTURE_SHAPES = (
    ...
    # NOT `^captures?/`. This repo COMMITS its captures, deliberately, and
    # the reasoning that put the pattern here is preserved by
    # `captures_are_scrubbed_and_trimmed()` below rather than discarded: what
    # must not be tracked is a RAW dump, and a scrubbed, trimmed one is a
    # fixture. `smoke_test.py` pins values against these files, and a fixture
    # that is not in the repo does not make the check fail — it makes it
    # SKIP, which reads as green.
...
def secret_check():
    ...
    failed.extend(captures_are_scrubbed_and_trimmed())

    # A scan that read nothing passes for the wrong reason, and this one did:
    # run before `git init`, `git ls-files` returned nothing and the check
    # printed "0 files scanned ... nothing credential-shaped". A guard whose
    # input has silently disappeared is the failure mode this file exists to
    # prevent, so it now refuses to pass on an empty scan.
    if scanned == 0:
        failed.append("the scan read ZERO files — `git ls-files` returned "
                      "nothing. Is this a git repository? A scan with no "
                      "input passes for the wrong reason.")

    if not failed:
        print(f"ok       {scanned} files scanned, nothing credential-shaped, "
              f"every tracked capture scrubbed and trimmed")
    return failed


# A tracked capture must be a FIXTURE, not a dump. Two things make it one, and
# both are checked here rather than trusted:
#
#   * it is scrubbed — no reviewer's 21-digit profile id, no avatar URL. Play
#     publishes a real person's name, picture and stable id beside their
#     words, and republishing those is a separate act from the store showing
#     them on its own page;
#   * it is trimmed — a raw Play response is 1.2 to 3.3 MB, and a repository
#     has no business carrying that. `make_fixtures.py` cuts each one to the
#     data blocks the parser reads and proves the result parses identically
#     before writing it back.
#
# Matched by PATTERN rather than against the values that happen to be in the
# current set, so the NEXT capture is caught too.
_REAL_PROFILE_ID = re.compile(r"[^\d](\d{20,22})[^\d]")
_REAL_AVATAR = re.compile(r"googleusercontent\.com/a-?/(?!VATAR-REDACTED)")
_CAPTURE_SIZE_CEILING = 2_000_000


def captures_are_scrubbed_and_trimmed():
    failed = []
    for path in tracked_files():
        rel = path.relative_to(REPO).as_posix()
        if not rel.startswith("captures/") or rel.endswith(".md"):
            continue
        size = path.stat().st_size
        if size > _CAPTURE_SIZE_CEILING:
            failed.append(f"{rel} is {size / 1e6:.1f} MB — a tracked capture "
                          f"must be trimmed to the blocks the parser reads. "
                          f"Run make_fixtures.py.")
        try:
            text = path.read_text(encoding="utf-8", errors="replace")
        except OSError:
            continue
        ids = {m.group(1) for m in _REAL_PROFILE_ID.finditer(text)
               if not m.group(1).startswith("1000000000000000000")}
        if ids:
            failed.append(f"{rel} carries {len(ids)} reviewer profile id(s). "
                          f"Run scrub_fixtures.py before committing it.")
        if _REAL_AVATAR.search(text):
            failed.append(f"{rel} carries a real reviewer avatar URL. "
                          f"Run scrub_fixtures.py before committing it.")
    return failed
