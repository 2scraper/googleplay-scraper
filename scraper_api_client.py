#!/usr/bin/env python3
"""googleplay-scraper — 2captcha Scraper API edition (fourth engine)
...
def parse_args():
    ...
    p.add_argument("--out", default="googleplay_apps_scraperapi", help="Output file prefix")
    ...
    env_config.apply(args, keys={
        "TWOCAPTCHA_KEY": "key",
        "GOOGLEPLAY_CDP_ENDPOINT": "cdp_url",
        "GOOGLEPLAY_URL": "url",
    })
    if not args.url:
        p.error("no --url given, and GOOGLEPLAY_URL is not set in the environment "
                "or in .env.")
    return args
